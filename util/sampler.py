...
def next_batch_pairwise(data,batch_size):
    if hasattr(data,'training_data'):
        training_data = data.training_data
    else:
        training_data = data.training_p
...
def next_batch_pairwise1(data,batch_size):
    training_data = data.training_c
    shuffle(training_data)
    batch_id = 0
    data_size = len(training_data)
    while batch_id < data_size:
        ...
        for i, user in enumerate(users):
            ...
            j_idx.append(data.item[neg_item])
        yield u_idx, i_idx, j_idx
def next_batch_pairwise2(data,batch_size):

    training_data = data.training_f
    shuffle(training_data)
    batch_id = 0
    data_size = len(training_data)
    while batch_id < data_size:
        ...
        for i, user in enumerate(users):
            ...
            j_idx.append(data.item[neg_item])
        yield u_idx, i_idx, j_idx
def next_batch_pairwise0(data,batch_size):

    training_data = data.training_v
    shuffle(training_data)
    batch_id = 0
    data_size = len(training_data)
    while batch_id < data_size:
