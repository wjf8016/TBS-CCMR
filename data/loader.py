...
class FileIO(object):
    ...
    def load_data_set(file, dtype):
        ...
        if dtype == 'MultiBehavior':
            lines = open(file, "r").readlines()
            for l in lines:
                tmps = l.strip()
                inters = [int(i) for i in tmps.split(" ")]
                user_id, item_id = inters[0], inters[1:]
                pos_ids = list(set(item_id))
                for i_id in pos_ids:
                    data.append([user_id, i_id])

        if dtype == 'multi_behavior':
            lines = open(file, "r").readlines()
            for l in lines:
                tmps = l.strip()
                inters = [int(i) for i in tmps.split(" ")]
